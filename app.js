...
app.put("/users/:id", (request, response) => {
  ...
  if (userIndex === -1) {
    return response
      .status(404)
      .json({ error: "Error: User with ID not found." });
  }
  ...
  if (!name && !email && !age) {
    return response
      ...
      .json({
        error:
          "Error: At least one field (name, email, age) must be present for this operation.",
      });
  }
...
app.delete("/users/:id", (request, response) => {
  ...
  if (userIndex === -1) {
    return response
      .status(404)
      .json({ error: "Error: No user found to delete." });
  }
